# RFID proje kutusu V2

Montajlı dış ölçü **150 × 110 × 55 mm**. Gövde 52 mm, kapak yüzeyi 3 mm; kapak ayakları kutunun içine girer. Tüm STL dosyaları **mm** birimindedir, %100 ölçekte dilimlenmelidir. Bu sürüm önceki kutuya ek parça değildir; V2 gövde ve V2 kapağı birlikte kullanın.

## Parçalar

| Dosya | Adet | Kullanım |
|---|---:|---|
| govde_v2.stl | 1 | ESP32 ve plaket için dört ayrı vida ayağı; iki LED, buzzer ve güç jakı yuvaları |
| kapak_v2.stl | 1 | LCD için dört vida ayağı ve pencere; RC522 için dört vida ayağı |
...
1. **ESP32:** Gövdenin sol arka bölümündeki dört ayağa takılır. Micro USB konektörü sol duvara bakar; 16 × 12 mm servis açıklığı USB kablosu içindir. Kart PCB altı tabandan 10 mm yüksektedir. Kapak açıkken BOOT/EN düğmelerine erişilir.
2. **Plaket:** Sağ arka bölümde dört M3 vida ile sabitlenir. Lehim yüzeyi altında 7 mm boşluk vardır. Ayak çevresinde iletken yolları vida başına temas ettirmeyin; gerekirse yalıtkan pul kullanın.
3. **LED'ler:** Gövdenin ön duvarına içeriden yerleştirilir, kubbe dışarı bakar. Yarıklı bilezik gövdeyi sıkar; LED'in taban flanşı içeride kalır.
4. **Buzzer:** Ön duvardaki büyük yarıklı yuvaya içeriden bastırılır. Ses veren yüz beş delikli ızgaraya, bacaklar içeri bakar. Yuva duvarı esneyerek tutar.
...
- Kapak: dış yüzü tabla üzerinde, ayaklar yukarı. Dosya zaten bu yöndedir.
...
- Gövdedeki yatay LED/buzzer yuvaları, jak kanalları ve USB açıklığı için **yerel destek gerekebilir**. Dilimleyicide destekleri inceleyin; küçük sıkıştırma yarıklarını destekle doldurmayın. Destekleri parçaları takmadan önce temizleyin.
- Kapak ve jak baskı parçası önerilen yönde destek gerektirmeyecek şekilde tasarlanmıştır.
- Önce `sikilik_testi.stl` basılabilir: soldan sağa LED çapları 4,8 / 4,9 / 5,0 mm; buzzer çapları 10,85 / 11,05 mm. Test parçasını düz yüzü altta kullanın. Gövdedeki yatay baskı yönü delik toleransını ayrıca etkileyebilir. Zorlayarak takmayın; gerekirse çapı kaynak dosyasında değiştirin.
