...
# Front-face LEDs: split collars grip 5 mm bodies. Lens insertion from interior.
for x in P['led']['x_positions']:
    z=P['led']['z']
    collar=along_y(x,wall-0.2,z,4.3,3.7)
    collar-=along_y(x,wall-0.3,z,P['led']['bore']/2,4)
    collar-=box(x-0.45,wall+0.3,z,0.9,4,5)
    base+=collar
    base-=along_y(x,-1,z,P['led']['exit']/2,wall+1.05)

# Buzzer split sleeve, face towards front grille, wire end toward inside.
b=P['buzzer']; bx=b['x']; bz=b['z']; sleeve_depth=b['height']
sleeve=along_y(bx,wall-0.2,bz,b['diameter']/2+1.7,sleeve_depth+0.2)
sleeve-=along_y(bx,wall-0.3,bz,b['bore']/2,sleeve_depth+0.6)
for dx in (-7.4,0):
    sleeve-=box(bx+dx,wall+1,bz-0.45,7.4, sleeve_depth,0.9)
base+=sleeve
for dx,dz in [(0,0),(-2.4,0),(2.4,0),(0,-2.4),(0,2.4)]:
    base-=along_y(bx+dx,-1,bz+dz,0.8,wall+2)
...
parts={'govde_v2':base,'kapak_v2':lid,'guc_jaki_baski_parcasi':clamp,'sikilik_testi':gauge}
report={}; meshes={}
...
lid_asm=lid.mirror((0,0,1)).translate((0,0,H+T))
...
for index,name in enumerate(['govde_v2','kapak_v2'],1):
    ...
    if index==1:
        labels=[(30,82,13,'ESP32 / 4 vida'),(115,80,13,'50×50 plaket / 4 vida'),(31,9,31,'2 LED'),(66,12,32,'Buzzer'),(140,27,27,'Güç jakı')]
    else: labels=[(75,84,13,'LCD / 4 vida'),(75,32,9,'RC522 / 4 vida')]
...
fig.suptitle('RFID KUTUSU V2 • 150 × 110 × 55 mm\nGerçek STL geometrisi — seçilen modül çizimlerine göre',fontsize=16)
...
for ax,title,nameset in [(axes[0],'Gövde: üstten montaj planı',['esp32','perfboard']),(axes[1],'Kapak: iç yüzden montaj planı',['lcd','rc522'])]:
    ...
    for name in nameset:
        ...
        for xx,yy in holes(p): ax.add_patch(Circle((xx,yy),1.5,color='#a54136'))
...
axes[0].annotate('Micro USB',xy=(0,82),xytext=(12,57),arrowprops={'arrowstyle':'->'})
...
axes[0].text(42,8,'Ön duvar: LED • LED • buzzer',ha='center',fontsize=8)
